...
/*   Updated: 2018/04/06 06:42:00 by jhamon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_awesome_rchr(t_filler *p, char c)
{
	...
	if (p->tb_map[y][x] != c)
		return (0);
	if ((x == p->x_map && y == 0) || (y == p->y_map && x == 0) || \
		(x == 0 && y == 0) || (x == p->x_map && y == p->y_map))
		return (ft_cas_prt(p, c));
	else if (f == 1 || f == 5 || f == 6 || f == -5 || f == -6 || f == -1)
	{
		if (p->tb_map[y - 1][x] == c)
		{
			if (p->tb_map[y + 1][x] == c)
	...
	else if (f == 2 || f == 4 || f == 3 || f == -4 || f == -3 || f == -2)
	{
		if (p->tb_map[y][x - 1] == c)
		{
			if (p->tb_map[y][x + 1] == c)
